...
router.post('/:id/attributes', async (req, res) => {
  ...
  if (!nombre || !valor || stock === undefined) {
    return res.status(400).json({ error: 'Nombre, valor y stock son requeridos' });
  }
  ...
  try {
          ...
          connection.query(
            ...
            (err, results) => {
              if (err) {
                return connection.rollback(() => {
                  connection.release();
                  console.error('Error al verificar atributo:', err);
                  return res.status(500).json({ error: 'Error al verificar atributo existente' });
                });
              }

              let idAtributo;
              if (results.length > 0) {
                idAtributo = results[0].ID_ATRIBUTO;
                insertProductoAtributo();
              } else {
                  ...
                  (err, result) => {
                    if (err) {
                      return connection.rollback(() => {
                        ...
                        return res.status(500).json({ error: 'Error al crear nuevo atributo' });
                      });
                    }
                    
                    idAtributo = result.insertId;
                    insertProductoAtributo();
                  }
                );
              }

              function insertProductoAtributo() {
                // Insertar en producto_atributos
                connection.query(
                  'INSERT INTO producto_atributos (ID_PRODUCTO, ID_ATRIBUTO, VALOR_ATRIBUTO) VALUES (?, ?, ?)',
                  [productId, idAtributo, valor],
                  (err, productoAtributoResult) => {
                    if (err) {
                      return connection.rollback(() => {
                        connection.release();
                        console.error('Error al asociar atributo con producto:', err);
                        return res.status(500).json({ error: 'Error al asociar atributo con producto' });
                      });
                    }
                    ...
                    connection.query(
                      ...
                      [idProductoAtributo, stock, stock],
                      (err) => {
                        if (err) {
                          return connection.rollback(() => {
                            connection.release();
                            console.error('Error al crear registro de stock:', err);
                            return res.status(500).json({ error: 'Error al crear registro de stock' });
                          });
                        }
                        ...
                        connection.commit((err) => {
                          ...
                          connection.release();
                          res.json({ 
                            message: 'Atributo agregado con éxito', 
                            newAttribute: {
                              ID_PRODUCTO_ATRIBUTO: idProductoAtributo,
    ...
    res.status(500).json({ error: 'Error al agregar atributo', message: error.message });
  }
});
// Agregar producto
